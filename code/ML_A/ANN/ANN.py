...
data = numpy.loadtxt(open('AAPL2.csv', 'rb'), delimiter = ',')#Load
numpy.random.shuffle(data)

features = data[:,0:11]
nfeatures = (features-features.mean())/features.std()
targets = numpy.matrix(data[:, 12]).T

n = NN([11, 50, 1])

n.train(nfeatures, targets, 0.07, 0.0, 200)

elapsed = (time.clock()-start)
print('AAPL ANN clock time ' + repr(elapsed))
...
matplotlib.pyplot.title('AAPL Data vs. Predicted')

matplotlib.pyplot.savefig('Report_AAPL_Neural.png', format = 'png')
...
data = numpy.loadtxt(open('GOOG2.csv', 'rb'), delimiter = ',')#Load
numpy.random.shuffle(data)

features = data[:,0:11]
nfeatures = (features-features.mean(axis=0))/features.std()
targets = numpy.matrix(data[:, 12]).T
...
elapsed = (time.clock()-start)
print('GOOG ANN clock time ' + repr(elapsed))
...
matplotlib.pyplot.scatter(n.predict(nfeatures).tolist(), targets.tolist())
matplotlib.pyplot.plot(targets.tolist(), targets.tolist(), c = 'r')
matplotlib.pyplot.title('GOOG Data vs. Predicted')

matplotlib.pyplot.savefig('Report_GOOG_Neural.png', format = 'png')
matplotlib.pyplot.close()
